...
class QuizInterface:
    def __init__(self, quiz_brain: QuizBrain):
        ...
        self.true_button = tkinter.Button(image=self.true_image, bg=THEME_COLOR, highlightthickness=0,
                                          command=self.true_pressed)
        self.true_button.grid(row=2, column=0)
        # wrong button
        self.false_image = tkinter.PhotoImage(file='images/false.png')
        self.false_button = tkinter.Button(image=self.false_image, bg=THEME_COLOR, highlightthickness=0,
    ...
    def get_next_question(self):
        ...
        if self.quiz.still_has_questions():
            ...
            self.canvas.itemconfig(self.question_text, text="You've reached the end onf the quiz.")
